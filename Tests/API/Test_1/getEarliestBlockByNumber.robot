...
*** Test Cases ***
Valid Test ETH Get Earliest Block By Number
     [Documentation]    Test obtaining earliest block details using eth_getBlockByNumber endpoint
     [tags]  Smoke
     Given user sets endpoint with base api ${NODE_URL}
     And user sets headers ${headers}
     And user sets body ${earliestBlock}
